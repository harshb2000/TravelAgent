...
def test_chat_returns_assistant_message():
    ...
    with patch("httpx.post", return_value=_make_response(expected)) as mock_post:
        result = client.chat([{"role": "user", "content": "hi"}], timeout=120.0)
...
def test_chat_passes_extra_headers():
    ...
    with patch("httpx.post", return_value=_make_response(expected)) as mock_post:
        client.chat([{"role": "user", "content": "hi"}], timeout=120.0)
...
def test_chat_recovers_from_tool_use_failed():
    ...
    with patch("httpx.post", return_value=_make_error_response("tool_use_failed", raw)):
        result = client.chat([{"role": "user", "content": "find flights"}], timeout=120.0)
...
def test_chat_merges_extra_body_into_request():
    client = LLMClient("https://api.example.com/v1", "key", "qwen3.5:9b")
    ok_msg = {"role": "assistant", "content": "hi"}
    with patch("httpx.post", return_value=_make_response(ok_msg)) as mock_post:
        client.chat([{"role": "user", "content": "hi"}], timeout=120.0, extra_body={"reasoning_effort": "none"})
...
def test_chat_omits_extra_keys_when_extra_body_not_given():
    client = LLMClient("https://api.example.com/v1", "key", "model-x")
    ok_msg = {"role": "assistant", "content": "hi"}
    with patch("httpx.post", return_value=_make_response(ok_msg)) as mock_post:
        client.chat([{"role": "user", "content": "hi"}], timeout=120.0)
...
def test_chat_sends_no_max_tokens_by_default():
    client = LLMClient("https://api.example.com/v1", "key", "model-x")
    ok_msg = {"role": "assistant", "content": "hi"}
    with patch("httpx.post", return_value=_make_response(ok_msg)) as mock_post:
        client.chat([{"role": "user", "content": "hi"}], timeout=120.0)
...
def test_chat_respects_max_tokens_via_extra_body_and_timeout_override():
    client = LLMClient("https://api.example.com/v1", "key", "model-x")
    ok_msg = {"role": "assistant", "content": "hi"}
    with patch("httpx.post", return_value=_make_response(ok_msg)) as mock_post:
        client.chat([{"role": "user", "content": "hi"}], timeout=30.0, extra_body={"max_tokens": 1024})
    body = mock_post.call_args.kwargs["json"]
...
def test_chat_sends_tools_when_provided():
    ...
    with patch("httpx.post", return_value=_make_response(expected)) as mock_post:
        client.chat([{"role": "user", "content": "hi"}], timeout=120.0, tools=tools)
    _, kwargs = mock_post.call_args
