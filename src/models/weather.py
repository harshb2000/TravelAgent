...
class DailyWeather(BaseModel):
    ...
    precipitation_prob: int | None = Field(
        description="Precipitation probability 0–100%. Populated in forecast mode; null in climate mode."
    )
    ...
    weather_description: str = Field(
        description="WMO weather description e.g. 'Rain: moderate'. Empty string in climate mode — use precipitation_sum instead."
    )
...
class WeatherOutput(BaseModel):
    ...
    city: str = Field(description="City name as provided")
    days: list[DailyWeather] = Field(
        description="Per-day data. Forecast: one entry per requested day. Climate: one entry per calendar day in the month."
    )
